...
- (bool)sendRegistrationRequestwithSender:(GCDAsyncSocket *)sender withFiles:(NSArray *) files
{
    ...
    for (NSString *file in files) {
        ...
        result = [self sendRequestwithSender:sender withData:[theFileName dataUsingEncoding:NSUTF8StringEncoding] withGoal:RECEIVED_MARKER];
        if (!result) {
       ...
       //
       // NSLog(theFileName);
        [self sendFilewithSender:sender withFiles:file];
    }
    
    // send EOF of all
    NSLog(@"send EOF FILE_ALL_END_HERE");
...
-(bool)sendFilewithSender:(GCDAsyncSocket *)sender withFiles:(NSString *)filename
{
    ...
    {
        UIImage *chosenImage = [UIImage imageWithContentsOfFile:filename];
    ...
    NSLog(@"Total length = %i", totalLen);
    //   NSData *piece = buffer;
    uint8_t *readBytes = (uint8_t *)[data bytes];
    
    while (index < totalLen) {
        //if ([outputStream hasSpaceAvailable]) {
        int indexLen =  max_byte_transfer;
...
- (bool)receiveAndSaveFileswithSender:(GCDAsyncSocket *)sender withFolder:(NSString *)folder withData:(NSData *)data;
{
    ...
    if (!have_read_the_message) {
        NSString *fileNum = [[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding];
